...
class ContractView:
    ...
    def display_contracts(self, contracts):
        ...
        for contract in contracts:
            ...
            table.add_row(str(contract.id), str(contract.client), str(contract.sales_contact),
                          str(contract.total_amount), str(contract.remaining_amount), str(contract.created_at), status)
    ...
    def update_contract(self, contract):
        ...
        total_amount = self.input.check_int(
            f"Total Amount ({contract.total_amount}) : ", updated=True) or contract.total_amount
